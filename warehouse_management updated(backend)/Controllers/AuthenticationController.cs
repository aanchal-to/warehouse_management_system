    ...
    public class AuthenticationController : ControllerBase
    {
        ...
        private async Task<RegisterResponse> RegisterAsync(RegisterRequest request)
        {
            try
            {
                ...
                if (userExists != null)
                {
                    ...
                    if (userRoles.Any())
                    {
                        return new RegisterResponse { Message = "User already has a role assigned and cannot change roles", Success = false };
                    }
                }
        ...
        private async Task<LoginResponse> LoginAsync(LoginRequest request)
        {
            try
            {
                ...
                if (!result) return new LoginResponse { Message = "Invalid email/password", Success = false };

               // var role = await _userManager.GetRolesAsync(user);
              //  if (!role.Contains(request.Role)) return new LoginResponse { Message = "Invalid role", Success = false };
                ...
                var claims = new List<Claim>
            {
                ...
                var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes("ThisIsAStrongKeyForHS256AlgorithmThisIsAStrongKeyForHS256Algorithm"));
