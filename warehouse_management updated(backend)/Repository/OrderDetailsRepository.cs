...
namespace warehouse_management.Repository
{
    public class OrderDetailsRepository : IOrderDetailsRepository
    {
        ...
        {
            return _orderDetails.Find<OrderDetails>(orderDetail => orderDetail.orderId == orderId).FirstOrDefault();
        }
        ...
        public OrderDetails GetOrderDetailsByCustomerId(string id)
        {
            return _orderDetails.Find<OrderDetails>(orderDetail => orderDetail.customerId == id).FirstOrDefault();
        }

        public IEnumerable<OrderDetails> GetOrdersByCustomerId(string id)
        {

            var orders = _orderDetails.Find(orderDetail => orderDetail.customerId == id).ToList();
